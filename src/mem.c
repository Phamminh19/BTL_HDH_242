...
/* Search for page table table from the a segment table (second table)*/
...
addr_t alloc_mem(uint32_t size, struct pcb_t * proc) {
	 ...
	 * */
	 if (proc->bp + num_pages * PAGE_SIZE > (1 << ADDRESS_SIZE)) {
		mem_avail = 0;
		printf("No space for alloc_mem");
	}
	
	int free_pages[num_pages];  // Store indices of free pages
    int count = 0;

    for (int i = 0; i < NUM_PAGES && count < num_pages; i++) {
	...
	if (mem_avail) {
		...
		for(int i = 0;i < num_pages;i++){
			int page_idx = free_pages[i];
			_mem_stat[page_idx].proc = proc->pid;
			_mem_stat[page_idx].index = i;
			_mem_stat[page_idx].next = (i == num_pages - 1) ? -1 : free_pages[i + 1];
			addr_t v_addr = ret_mem + i * PAGE_SIZE;
    		...
    		trans_table->table[trans_table->size].p_index = page_idx;
    		trans_table->size++;
		}
	}
