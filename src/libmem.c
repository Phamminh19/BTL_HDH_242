 ...
 */
 ...
 int __alloc(struct pcb_t *caller, int vmaid, int rgid, int size, int *alloc_addr)
 {
   ...
   int inc_sz = PAGING_PAGE_ALIGNSZ(size);
    ...
    /* INCREASE THE LIMIT by invoking systemcall 
    ...
    /* Try to get free area again after increasing limit */
    if (get_free_vmrg_area(caller, vmaid, size, &rgnode) == 0)
    {
 ...
 int pg_getval(struct mm_struct *mm, int addr, BYTE *data, struct pcb_t *caller)
 {
    ...
    */
   int phyaddr = (fpn* PAGING_PAGESZ) + off; // fpn * pagesize + off
 ...
 int __read(struct pcb_t *caller, int vmaid, int rgid, int offset, BYTE *data)
 {
   ...
   if (currg == NULL || cur_vma == NULL) /* Invalid memory identify */
    ...
    int res = pg_getval(caller->mm, currg->rg_start + offset, data, caller);
